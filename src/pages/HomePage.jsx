...
function HomePage() {
    ...
    const [translatedQuote, setTranslatedQuote] = useState(""); // Cita traducida

    // Función para traducir la cita usando LibreTranslate
    const translateQuote = async (quote) => {
        try {
            const response = await fetch("https://libretranslate.de/translate", {
                ...
                body: JSON.stringify({
                    q: quote,         // La cita original
                    source: "en",     // Idioma original (inglés en este caso)
                    target: "es",     // Traducir a español (puedes cambiarlo a cualquier idioma)
                }),
            });

            const data = await response.json();
            return data.translatedText; // Retorna la cita traducida
        } catch (error) {
            console.error("Error al traducir:", error);
            return quote; // Si hay un error, devuelve la cita original
        }
    };

    // Llamada a la API de citas y traducción
    useEffect(() => {
        const fetchQuote = async () => {
            ...
            if (savedQuote && savedDate === today) {
                ...
                setLoading(false);
                setTranslatedQuote(await translateQuote(savedQuote)); // Traducir la cita
            } else {
                try {
                    ...
                    const data = await response.json();
                    setQuote(data[0].q); // Cita original
                    setAuthor(data[0].a); // Autor de la cita

                    // Guardar la nueva cita y la fecha en localStorage
                    localStorage.setItem("dailyQuote", data[0].q);
                    localStorage.setItem("dailyAuthor", data[0].a);
                    localStorage.setItem("quoteDate", today);

                    // Traducir la cita obtenida
                    const translated = await translateQuote(data[0].q);
                    setTranslatedQuote(translated); // Actualizar estado de la cita traducida
                } catch (error) {
                    console.error("Error:", error);
                    setQuote("La inspiración también viene de nuestros propios pasos.");
                    setAuthor("Vida Propia");
                    setTranslatedQuote("La inspiración también viene de nuestros propios pasos.");
                } finally {
                    setLoading(false);
                }
            }
                ...
                <div className="container">
                    ...
                    {loading ? (
                        ...
                        <p className="lead fst-italic mt-3">
                            <strong>Cita del día:</strong> "{translatedQuote || quote}"  {/* Mostrar la cita traducida si existe, sino mostrar la original */}
